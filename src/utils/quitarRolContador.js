...
module.exports = (client) => {
  ...
  setInterval(async () => {
    ...
    if (date.getDay() === 5) {
      ...
      const membersWithRole = guild.members.cache.filter(member => member.roles.cache.has(process.env.ROLE_ID_CONTADOR));
      ...
      if (membersWithRole.size === 1) {
        ...
        await channel.send(`${mentions} Ya podés volver a escribir en el contador.`);
      }
