...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    const canalAnonimo = interaction.guild.channels.cache.find(channel => channel.id === idCanalAnonimo);
    
    if (canalAnonimo) {
      canalAnonimo.send(`Mensaje anónimo: ${mensajeAnonimo}`).catch(error => {
        console.error('Error al enviar el mensaje anónimo:', error);
      });
      await interaction.editReply({ content:'¡Mensaje enviado de forma anónima!', ephemeral: true });
    } else {
      await interaction.editReply({ content:'No se pudo enviar el mensaje de forma anónima.', ephemeral: true });
    }
  }
};